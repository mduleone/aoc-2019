...
def create_updated_input_with_noun_verb(input, noun, verb)
  input.map.with_index do |ele, idx|
    if idx == 1
      noun
    elsif idx == 2
      verb
...
def run_program(operating_input)
  position = 0
...
while noun <= 99
  while verb <= 99
    puts noun * 100 + verb if run_program(create_updated_input_with_noun_verb(input, noun, verb)) == 19690720
    verb += 1
  end
  noun += 1
  verb = 0
